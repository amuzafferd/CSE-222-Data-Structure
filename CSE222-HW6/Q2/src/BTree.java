...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    /**
     *
     * @param item Aranacak data
     * @param data Sıralı dizi
     * @param first Hangi aralıkta aranacağına dair ilk bilgi
     * @param last Hangi aralıkta aranacağına dair ikinci bilgi
     * @return Bulunulan index
     */
    private int binarySearch(E item, E[] data, int first, int last) {
        ...
        if (last - first == 1) {
            if (item.compareTo(data[first]) <= 0)
                return first;
            else
                return last;
        }
        ...
        int compResult = item.compareTo(data[middle]);
        if (compResult == 0) {
            return middle;
        }
        if (compResult < 0) {
            return binarySearch(item, data, first, middle);
        } else {
